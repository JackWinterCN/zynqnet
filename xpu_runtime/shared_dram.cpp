...
off_t SHARED_DRAM_BASE_ADDR = 0x38000000;
int SHARED_DRAM_FD = -1;
volatile uint32_t* SHARED_DRAM_PTR = NULL;
...
bool SHARED_DRAM_open() {
  ...
  try {
      ...
      *reinterpret_cast<uint32_t*>(virt_mem) = test_value;
      ...
      SHARED_DRAM_PTR = (volatile uint32_t*)(virt_mem);
      SHARED_DRAM_BASE_ADDR = LockedMemoryManager::getPhysicalAddress(virt_mem);
      // 6. 释放内存
  ...
  printf("SHARED_DRAM_PTR=0x%lX\n", (unsigned long)SHARED_DRAM_PTR);
  printf("SHARED_DRAM_BASE_ADDR=0x%lX\n", (unsigned long)SHARED_DRAM_BASE_ADDR);
...
bool SHARED_DRAM_close() {
  ...
  // std::cout << "\n=== Freeing Locked Memory ===" << std::endl;
  // LockedMemoryManager::free((void*)SHARED_DRAM_PTR, MEM_SIZE);
  // std::cout << "Memory freed successfully" << std::endl;
...
volatile uint32_t* SHARED_DRAM_virtual() {
  return (volatile uint32_t*) SHARED_DRAM_PTR;
}

volatile uint32_t* SHARED_DRAM_physical() {
  return (volatile uint32_t*) SHARED_DRAM_BASE_ADDR;
}
...
volatile uint32_t* map_SHARED_DRAM(off_t base_addr) {
  printf("XFPGA Driver: map shared DRAM at base address 0x%lX\n", (unsigned long)base_addr);
  ...
  volatile uint32_t* pointer = (uint32_t*)mmap(NULL, SHARED_DRAM_MEM_SIZE, PROT_READ | PROT_WRITE,
...
void release_SHARED_DRAM(volatile uint32_t* pointer) {
